...
namespace App\Filament\Electives\Resources\Fwpms\Schemas;
                    ...
                    ->schema([
                        Flex::make([
                            ImageEntry::make('professor_avatar')
                                ...
                                ->circular()
                                ->size(64)
                                ->state(fn (Fwpm $record): ?string => $record->professor?->getFilamentAvatarUrl())
                                ->grow(false)
                                ->visible(fn (Fwpm $record): bool => $record->professor?->getFilamentAvatarUrl() !== null),
                            ...
                            Grid::make(1)
                                ->schema([
                                    ...
                                    TextEntry::make('professor.email')
                                        ->label('Email')
                                        ->url(fn (Fwpm $record): ?string => $record->professor ? "mailto:{$record->professor->email}" : null)
                                        ->placeholder('-'),
                                ]),
                        ])->from('sm'),
                    ])
                        ...
                        Grid::make(2)
                            ->schema([
                                TextEntry::make('user_enrollment_status')
                                    ->label('Your Status')
                                    ->state(function (Fwpm $record): string {
                                        /** @var User|null $user */
                                        $user = Auth::user();
                                ...
                                TextEntry::make('enrolled_count')
                                    ...
                                    ->state(function (Fwpm $record): int {
                                        $semesterService = resolve(SemesterService::class);
                                        $currentSemester = $semesterService->getCurrentSemester();
